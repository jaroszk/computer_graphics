...
namespace cg_lab1
{
    class ImageUtil
    {
        ...
        public static Bitmap convolutionFilter(Bitmap bmpInput, List<List<int>> matrix, int offset, int anchor, int divisor)
        {
            Bitmap output = new Bitmap(bmpInput.Width, bmpInput.Height);
            Color c;
            float sumR = 0, sumG = 0, sumB = 0;
            ...
            for (int i = xSide; i <= bmpInput.Width - 1 - xSide; i++)
                for (int j = ySide; j <= bmpInput.Height - 1 - ySide; j++)
                {
                    ...
                    int colorR = (int)Math.Round(sumR / divisor);
                    int colorG = (int)Math.Round(sumG / divisor);
                    int colorB = (int)Math.Round(sumB / divisor);
